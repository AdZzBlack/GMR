...
class Maps extends REST_Controller { 
	...
	function getMaps_post(){     
		...
		$nomor_bangunan          = (isset($jsonObject["nomor_bangunan"])  	? $jsonObject["nomor_bangunan"]                    : "19");
            ...
            foreach ($result->result_array() as $r){
				...
				if($r['isparent']==1)
				{
					$query1 = "SELECT 
								...
								GROUP_CONCAT( FC_GENERATE_PROGRESSBANGUNAN(a.nomor) SEPARATOR '@') AS progress_bangunan,
								GROUP_CONCAT(FC_GENERATE_PROGRESSNAMA(a.nomor) SEPARATOR '@') AS progress_nama
							 FROM mhbangunan a 
							 WHERE a.status_aktif > 0 AND a.koordinat IS NOT NULL AND a.nomorheader = ".$nomor_bangunan;
