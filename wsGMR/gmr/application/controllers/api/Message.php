...
class Message extends REST_Controller { 
	...
	function sendPrivateMessage_post(){     
        ...
        if ($this->db->trans_status() === FALSE){
			...
			$vcGCMId = $this->getGCMId($userto_nomor);
			$nama = $this->db->query("SELECT a.nama FROM mhadmin a WHERE a.nomor = $userfrom_nomor")->row()->nama;
        	$this->send_gcm($vcGCMId, $this->ellipsis($new_message),'New Message(s) From ' . $nama,'PrivateMessage',$userfrom_nomor,'0');
        }  
	...
	function getPrivateMessage_post(){     
        ...
        if( $result && $result->num_rows() > 0){
            foreach ($result->result_array() as $r){
				$userfrom_nama = $this->db->query("SELECT nama FROM mhadmin where nomor ='". $r['nomoruser_from']."'")->row()->nama;
                $userto_nama = $this->db->query("SELECT nama FROM mhadmin where nomor ='". $r['nomoruser_to']."'")->row()->nama;
