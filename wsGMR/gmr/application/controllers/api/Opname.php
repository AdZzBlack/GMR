...
class Opname extends REST_Controller { 
	...
	function createOpname_post(){     
		...
		if($progress==100)
		{
			$query1 = "UPDATE mdrab set progress = $progress, status_selesai = 1 WHERE nomor = $nomor_rab" ;
			$this->db->query($query1);
		}
		else
		{
	...
	function createCreditNote_post(){     
		...
		$pieces = explode("|", $nomor);
		for($i=0;$i<count($pieces);$i++)
		{
		    if($pieces[$i] != "")
		    {
		        $query3 = "UPDATE thopname SET status_notabeli = 2 WHERE nomor = " . $pieces[$i];
		...
		$query = "  SELECT
						...
						d.nomormhcabang AS cabang
					FROM thopname a
					JOIN mdrab c ON a.nomormdrab = c.nomor
					JOIN mhbangunan d ON a.nomormhbangunan = d.nomor
					WHERE a.status_notabeli = 2
		...
		if( $result && $result->num_rows() > 0){
            foreach ($result->result_array() as $r){
				...
				$query1 = "  SELECT
								a.nomor AS nomor,
								d.nama AS nama,
								(a.progress - IFNULL((SELECT (b.progress) FROM thopname b WHERE b.nomor < a.nomor AND b.nomormdrab = a.nomormdrab AND b.status_aktif = 1 ORDER BY b.nomor DESC LIMIT 1),0))/100 AS qty,
								c.subtotal AS harga,
								(a.progress - IFNULL((SELECT (b.progress) FROM thopname b WHERE b.nomor < a.nomor AND b.nomormdrab = a.nomormdrab AND b.status_aktif = 1 ORDER BY b.nomor DESC LIMIT 1),0))/100 * c.subtotal AS subtotal
							FROM thopname a
							JOIN mdrab c ON a.nomormdrab = c.nomor
	...
	function deleteCreditNote_post(){
        $data['data'] = array();
        ...
        $query3 = "UPDATE thopname a
                    JOIN tdnotabeli b ON b.nomorthopname = a.nomor
                    JOIN thnotabeli c ON b.nomorthnotabeli = c.nomor
                    SET status_notabeli = 0 WHERE c.nomor = $nomor";
        $this->db->query($query3);

        $query2 = "UPDATE thnotabeli
                    SET status_aktif = 0,
                        dihapus_oleh = $nomor_user,
                        dihapus_pada = NOW()
                    WHERE nomor = $nomor";
        $this->db->query($query2);

        $query1 = "UPDATE tdnotabeli
                    SET status_aktif = 0,
                        dihapus_oleh = $nomor_user,
                        dihapus_pada = NOW()
                    WHERE nomorthnotabeli = $nomor";
        $this->db->query($query1);

        $query0 = "UPDATE tlaporanhutang
                    SET status_aktif = 0,
                        dihapus_oleh = $nomor_user,
                        dihapus_pada = NOW()
                    WHERE intNomorTransaksi = $nomor";
...
//    		$query = "UPDATE thopname SET progress = $input WHERE nomor = $nomor";
            $nomormdrab = $this->db->query("SELECT nomormdrab FROM thopname WHERE nomor = $nomor")->row()->nomormdrab;

    		$query = "DELETE FROM thopname WHERE nomor = $nomor";
            $this->db->query($query);

            $progress = $this->db->query("SELECT IFNULL(progress,0) AS progress FROM thopname WHERE nomormdrab = $nomormdrab ORDER BY nomor DESC LIMIT 1")->row()->progress;
            if($progress==null) $progress = 0;
            $query = "UPDATE mdrab SET progress = $progress WHERE nomor = $nomormdrab";
            $this->db->query($query);
